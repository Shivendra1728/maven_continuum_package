...
public class ContinuumSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	@Autowired
	public ContinuumSecurityConfig(LoginService loginService, JwtTokenUtil jwtTokenUtil) {
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().authorizeRequests().antMatchers("/*").permitAll().antMatchers("/admin").hasRole("Admin")
				.antMatchers("/login").hasRole("USER") // Restrict access to /login endpoint to users with the "USER"
														// role
	...
	protected UserDetailsService userDetailsService() {
		return username -> {
			User user = loginService.getUserByUsernameOrEmail(username); // Retrieve user by username or email
			String password = "";
			if (user != null) {
